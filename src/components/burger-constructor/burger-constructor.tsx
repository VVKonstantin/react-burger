...
import { ADD_INGREDIENT_TO_BURGER, ADD_BUN_TO_BURGER, CLEAR_BURGER } from '../../services/actions/burger';
import { INC_COUNTER, SET_BUN_COUNTER, CLEAR_COUNTERS } from '../../services/actions/ingredients';
import { CLEAR_ORDER } from '../../services/actions/order';
import { createUniqueId } from '../../utils/funcs';
import Modal from '../modal/modal';
import { setOrder } from '../../services/actions/order';
import OrderDetails from "../order-details/order-detailes";
import BurgerItem from '../burger-item/burger-item';
import styles from './burger-constructor.module.css';
import { useNavigate } from 'react-router-dom';
import { TIngredient, TIngredientInBurger } from '../../services/types/data';
...
function BurgerConstructor() {
  ...
  const handleClickOrder = () => {
    ...
    ingredientsInBurger.map((elem: TIngredientInBurger) => ingredientIds.push(elem._id));
    ingredientIds.push(bun[0]._id);
    dispatch({
      type: CLEAR_ORDER
    })
  ...
  const [{ isHover }, dropRef] = useDrop({
    accept: 'items',
    drop(elem: TIngredient) {
      const item = createUniqueId(data.filter((el: TIngredient) => el._id === elem._id)[0]);
      if (item.type !== 'bun') {
        dispatch({
  ...
  const calcTotalPrice = React.useCallback(() => {
    const price = ingredientsInBurger.reduce((acc: number, item: TIngredient) => acc += item.price, 0) + 2 * bun.reduce((acc, item) => acc += item.price, 0);
    setTotalPrice(price);
      ...
      <ul className={`${styles.list} ml-4`} ref={dropRef} style={{ borderColor }}>
        ...
        <li className={`${styles.item}`}>
          <ul className={`${styles.list} ${styles.sublist}`}>
            ...
            {ingredientsInBurger.map((item: TIngredientInBurger, index: number) =>
